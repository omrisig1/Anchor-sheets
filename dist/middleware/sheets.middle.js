/* eslint-disable @typescript-eslint/unbound-method */ /* eslint-disable @typescript-eslint/no-unsafe-member-access */ /* eslint-disable @typescript-eslint/no-unsafe-argument */ import config from ".././config.js";
...
export async function validateSheetSchema(req, res, next) {
    ...
    if (!req.body.columns || !Array.isArray(req.body.columns)) {
        ...
        throw new validationException(400, `Field columns does not exist or not an array.`);
    }
    ...
    for (const column of req.body.columns){
        ...
        if (!column.name || !column.type) {
            log.red(`${method} - validationException`);
            throw new validationException(400, `Field ${JSON.stringify(column)} is missing name or type keys.`);
        }
        // Check if 'name' is alphabetic
        if (!/^[A-Za-z]+$/.test(column.name)) {
            log.red(`${method} - validationException`);
            throw new validationException(400, `Field ${column.name} under 'name' is not alphanumeric.`);
        }
        ...
        if (!allowedTypes.includes(column.type)) {
            log.red(`${method} - validationException`);
            throw new validationException(400, `Field ${column.type.toString()} under 'type' is not one of the allowed types : ${config.allowed_types}`);
        }
    }
    log.cyan(`${method} - end`);
    next();
}
export async function validateCellType(req, res, next) {
    ...
    if (col) {
        ...
        if (schema_type != typeof req.body["value"]) {
            log.red(`${method} - validationException`);
            throw new validationException(400, `Field value ${req.body["value"]} is not allowed in this column, should be of type :  ${schema_type}.`);
        }
    } else {
        log.red(`${method} - validationException`);
        throw new validationException(400, `Column does not exists under schema rules for this sheet, run get sheet to see schema.`);
    }
    log.cyan(`${method} - end`);
    next();
}
export async function validateSheetIndex(req, res, next) {
    ...
    if (!sheet) {
        log.red(`${method} - validationException`);
        throw new validationException(400, `Sheet ${id} does not exists.`);
    }
    log.cyan(`${method} - end`);
    next();
}
export async function validateLookUp(req, res, next) {
    ...
    if (!col) {
        log.red(`${method} - validationException`);
        throw new validationException(400, `Lookup Cell/Column does not exists.`);
    }
    ...
    if (isLookUp(value)) {
        while(isLookUp(value)){
            ...
            if (formattedColumnName + lookup_cell_coor[1] == col.name + asking_cell_number) {
                log.red(`${method} - validationException`);
                throw new validationException(400, `Circular Reference found.`);
        ...
        if (typeof value != schema_type_allowed) {
            log.red(`${method} - validationException`);
            throw new validationException(400, `Lookup Field value ${value} is not allowed in this column, should be of type : ${schema_type_allowed}.`);
        }
    }
    log.cyan(`${method} - end`);
    next();
